...
import { getAnalysisResultById } from 'apiSdk/analysis-results';
...
function AnalysisResultViewPage() {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('analysis_result', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/analysis-results/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              {hasAccess('dns_log', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/dns-logs/view/${data?.dns_log?.id}`}>
                      {data?.dns_log?.log_data}
                    </Link>
                  </Text>
                </Flex>
              )}
